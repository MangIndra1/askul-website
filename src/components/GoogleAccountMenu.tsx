...
import { disconnectGoogleCalendar, syncAllTasksToGoogle } from '@/app/actions/googleCalendar'
...
export function GoogleAccountMenu({
  ...
  async function handleSync() {
    ...
    setSyncing(false)
    setSyncMessage(result.error ? 'Sync gagal, coba lagi' : `${result.synced} tugas berhasil disinkron ✓`)
    router.refresh()
  }
  ...
  return (
    <div className="flex w-full flex-col gap-2.5">
      ...
      {/* Banner sync — cuma nongol kalau ada tugas lama yang belum
          tersinkron. Hilang sendiri begitu semuanya beres. */}
      {(unsyncedCount > 0 || syncMessage) && (
        <div className="glass flex items-center justify-between gap-3 rounded-2xl px-4 py-3">
          <span className="text-xs text-[var(--dk-text-soft)]">
            {syncMessage ?? `${unsyncedCount} tugas lama belum tersinkron ke Google`}
          </span>
          {!syncMessage && (
            <button
              onClick={handleSync}
              disabled={syncing}
              className="shrink-0 rounded-full bg-[var(--lav-600)] px-3 py-1.5 text-xs font-semibold text-white transition disabled:opacity-50"
            >
              {syncing ? 'Nyinkron...' : 'Sync sekarang'}
            </button>
          )}
        </div>
      )}
